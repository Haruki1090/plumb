...
| 中身 | 型 | 夜を跨ぐなら |
|---|---|---|
| 計画をタスク単位で実行する（独立・同一セッション） | `superpowers:subagent-driven-development` | **終了述語とチェックポイントはこちらが持つ** |
| 一つの測れる指標を押し下げ続ける | `playbooks/hillclimb.md` | 同上。停止条件は hillclimb が上書きする |
| どれでもない長い仕事 | **この型だけ** | |

**「計画ファイルを作ったから SDD へ」で降りられない。**SDD の適用条件は
「計画がある」ではなく「計画があり、タスクがおおむね独立で、同じセッションで回す」。
...
5. **反復ごとにチェックポイントを残す。**
   ...
   scripts/decision-log.sh <ログ> --header 反復 やったこと なぜ 根拠 述語が動いたか
   scripts/decision-log.sh <ログ> 0 "述語を固定" "<述語の全文>" "-" "-"
   scripts/decision-log.sh <ログ> 1 "<やったこと>" "<なぜ>" "<根拠>" yes
   ```
