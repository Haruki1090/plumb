...
> **これは `superpowers:brainstorming` の Spike の中で使う道具であって、その代わりではない。**
> Spike の関門は残る——**何を試すかを 2〜3 文で提示し、頷きをもらうまで作らない。**
> 「自分で答えられるから聞かない」は、**作ってよいかの承認には適用されない。**
> 承認の後は、Spike が「安く済ませろ」と言うところを、この型が具体的な作り方で埋める。
...
1. **このプロトタイプが下すための判断を先に決める。**どの配置か、どの操作か、どの密度か。
   ...
   **判断が無いならプロトタイプはしない。**`superpowers` の実装ループへ回す。
...
6. 案・trade-off・推奨を出す。**成果物は判断と捨てる実装であって、出荷できるコードではない。**
   選ばれた方向は `superpowers` の実装ループへ渡して、本番として作り直す。
