...
| `~/.cursor/plugins/` | `~/.claude/skills/`（開発時に手で置く場合の設置場所） | pstack はプラグインとして `~/.cursor/plugins/` 配下に置かれていたが、plumb を手で `~/.claude/skills/plumb/` 配下に置くとそこが実体になる。**marketplace 経由でインストールした場合はここではなく `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/` が実体になる** |
